...
parser.add_argument("cc_scheme", help="cc scheme")
parser.add_argument("base_folder", help="test base folder")
parser.add_argument("num_columns", type= int, default= 5, help="num columns in the big figure")
parser.add_argument("fig_col", type= int, default= 15, help="col size in the big figure, fig_col/fig_row = 6/5")
parser.add_argument("fig_row", type= int, default= 12.5, help="row size in the big figure, fig_col/fig_row = 6/5")
parser.add_argument("--dpi", type= int, default= 50, help= "the dpi of big figure")
begin_time = datetime.datetime.now()
args = parser.parse_args()
cc_scheme = args.cc_scheme
base_folder = args.base_folder
num_columns = args.num_columns
fig_col = args.fig_col
fig_row = args.fig_row
...
num_figures = num_results // 60 if num_results % 60 == 0 else num_results // 60 + 1
...
for row, subfolder in enumerate(subfolders):
    ...
    if((row+1) % 60 == 0):
        print("merge results {}-{}".format(row+1-59, row+1))
        process_time = datetime.datetime.now()
        print("results {}-{} are merged, start to save the big figure, use time: {}".format(row+1-59, row+1, process_time - begin_time))
        plt.savefig(os.path.join(base_folder, os.pardir, 'output_image_{}-{}.png'.format(row+1-59, row+1)), dpi= args.dpi)  # 保存最终的图片
