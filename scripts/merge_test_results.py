...
parser.add_argument("fig_col", type= float, default= 24, help="col size in the big figure, fig_col/fig_row = 6/5")
parser.add_argument("fig_row", type= float, default= 24, help="row size in the big figure, fig_col/fig_row = 6/5")
parser.add_argument("--dpi", type= int, default= 100, help= "the dpi of big figure")
...
for row, subfolder in enumerate(subfolders):
    ...
    axes[ax_row, ax_col].set_title(subfolder, fontsize=36)  # 设置子图标题为子文件夹名字
    ...
    if((row+1) % 40 == 0 or row == num_results-1):
        ...
        plt.savefig(os.path.join(base_folder, os.pardir, 'output_image{}.png'.format(row+1)), dpi= args.dpi)  # 保存最终的图片
