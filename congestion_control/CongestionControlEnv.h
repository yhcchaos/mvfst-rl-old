...
class CongestionControlEnv {
 public:
  ...
  struct History {
    float action;  // Past action taken

    History(const float a) : action(a) {}
  };
 ...
 protected:
   ...
   * asynchronously.
   */
  virtual void onObservation(Observation&& obs, float reward, float episodeWeight) = 0;
 ...
 private:
  ...
  torch::Tensor lastSummaryMean_{torch::zeros({1}, torch::kFloat32)};
